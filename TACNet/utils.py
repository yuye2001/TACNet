...
r1 = [56.5, 57.8, 59.6, 60.4, 59.8, 59.2, 58.3]    # R-1
mAP = [54.7, 55.9, 57.7, 58.5, 57.9, 57.3, 56.4]   # mAP
...
def get_knn_score(features, labels, k=10):
    """
    Compute KNN consistency score for each sample.
    Args:
        features: (N, d) tensor or numpy array
        labels: (N,) tensor or numpy array
        k: number of neighbors
    Returns:
        scores: (N,) numpy array, range [0, 1]
    """
    ...
    res = faiss.StandardGpuResources()
    index = faiss.GpuIndexFlatIP(res, d) # Inner Product (cosine similarity for normalized vectors)
    index.add(features)
    
    # Search
    # D: distances, I: indices
    D, I = index.search(features, k + 1) # +1 because the query itself is included
    
    # Calculate consistency
    scores = np.zeros(N, dtype=np.float32)
    for i in range(N):
        # Exclude the sample itself (usually the first one, but check to be safe)
        neighbors_idx = I[i, 1:] # Take k neighbors
        neighbors_labels = labels[neighbors_idx]
        current_label = labels[i]
        
        # Count how many neighbors have the same label
        match_count = np.sum(neighbors_labels == current_label)
        scores[i] = match_count / k
